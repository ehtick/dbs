...
fontLoader.load(PATH_PREFIX + '/fonts/helvetiker_regular.typeface.json', font => {
...
new XYZLoader().load(PATH_PREFIX + '/lawn.xyz', display);
...
const loader = new GLTFLoader();
  ...
  dracoLoader.setDecoderPath(PATH_PREFIX + '/');
...
const loadHouse = () => {
  loader.load(PATH_PREFIX + '/small_house/scene.gltf', gltf => {
...
export function loadLogo() {
  loader.load(PATH_PREFIX + '/maple_tree/tree.glb', gltf => {
    const scene = gltf.scene;
    scene.position.x = 5;
    scene.traverse(child => {
        if (typeof child.castShadow == 'boolean') {
